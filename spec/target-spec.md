...
| Corner grid | **[TBD]** | Process corners for sky130's 1.8 V-core MOS flavor are not yet enumerated in this repo. `sky130-bandgap`'s corner set (`tt, ff, ss, fs, sf` plus resistor/bipolar corners, per its ratified spec and `spec/topology-survey.md`) is a likely template for the *shape* of the grid, but that sibling's own ratified spec runs its devices at 3.3 V, not this block's 1.8 V-core primary — so its specific corner-model file names do not transfer without a sky130 1.8 V-core device check. See [`porting-plan.md`](porting-plan.md) §2. |
